...
- **Infra deploy wiped custom domain bindings**: The Bicep deploy reset the Container App to its Bicep-declared state, which didn't include `customDomains` (they were configured via portal). The managed certs survived but had to be re-bound via `az containerapp hostname bind`. **Follow-up**: Add custom domain + cert config to `containerapp.bicep` to prevent this on future infra deploys.
